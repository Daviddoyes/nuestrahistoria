...
import { Camera, Plus, X, ListTodo, BookImage, Trophy, Globe } from 'lucide-react'
...
export default function PlanesPage() {
  ...
  return (
    ...
    >
      ...
      {/* ── Barra inferior: pestañas internas + vuelta a la app ── */}
      <nav
        className="fixed bottom-0 left-0 right-0 z-20 bg-[#0A0A0A] border-t border-[#1A1A1A] flex"
        style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
      >
        {([
          { id: 'planes' as const, label: 'Planes', Icon: ListTodo },
          { id: 'historias' as const, label: 'Historias', Icon: BookImage },
        ]).map(({ id, label, Icon }) => {
        ...
        <Link
          href="/mis-gooals"
          style={{ height: 56 }}
          className="flex-1 flex flex-col items-center justify-center gap-1 text-[#444444] active:bg-[#141414] transition-colors"
        >
          <Trophy className="w-6 h-6" strokeWidth={1.5} />
          <span className="text-[10px] uppercase tracking-wider leading-none">Mis Gooals</span>
        </Link>
      </nav>
